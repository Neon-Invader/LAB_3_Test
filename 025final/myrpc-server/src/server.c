...
int user_allowed(const char *username) {
    // Используем парсер конфигурации для чтения файла
    Config config = parse_config("/home/user/Загрузки/thirdlect/user.conf");
    
    // Проверяем, совпадает ли имя пользователя с указанным в конфиге
